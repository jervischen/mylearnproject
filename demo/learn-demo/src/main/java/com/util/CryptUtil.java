...
public class CryptUtil {
    ...
    public static String encrypt(String value, byte[] key) {
        try {
            return byte2hex(CryptUtil.encrypt(value.getBytes("UTF-8"), key));
    ...
    public static byte[] hexToBuffer(String str) {
        ...
        for (int i = -1; ++i < buffer.length;) {
            buffer[i] = (byte) (Character.getNumericValue(str.charAt(i * 2)) << 4 | Character.getNumericValue(str.charAt(i * 2 + 1)));
        }
        return buffer;
    }
    ...
    public static String decrypt(String value, byte[] key) {
        try {
            if (value == null || value.length() == 0 || value.length() % 8 !=0)
                return null;

            return new String(CryptUtil.decrypt(hexToBuffer(value), key), "UTF-8");
